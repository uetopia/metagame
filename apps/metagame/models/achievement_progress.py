...
class AchievementProgress(ndb.Model):
    ...
    #seasonCharacterKeyId = ndb.IntegerProperty(indexed=False)
...
class AchievementProgressResponse(messages.Message):
    """ a Progress's data """
    key_id = messages.IntegerField(1, variant=messages.Variant.INT32)
    title = messages.StringField(2)
    description = messages.StringField(3)
    iconUrl = messages.StringField(4)
    first = messages.BooleanField(5)
    season_first = messages.BooleanField(6)
...
class AchievementProgressRequest(messages.Message):
    """ a Achievement Progress updates """
    key_id = messages.IntegerField(1, variant=messages.Variant.INT32)
    title = messages.StringField(2)
    description = messages.StringField(3)
    iconUrl = messages.StringField(4)
...
ACHIEVEMENT_PROGRESS_RESOURCE = endpoints.ResourceContainer(
    AchievementProgressRequest
)

class AchievementProgressCollection(messages.Message):
    """ multiple Progress """
    achievement_progrogresses = messages.MessageField(AchievementProgressResponse, 1, repeated=True)
    response_message = messages.StringField(40)
    response_successful = messages.BooleanField(50)

class AchievementProgressCollectionRequest(messages.Message):
    """ a Achievement Progress collection request's data """
    ...
    awarded = messages.BooleanField(6)

ACHIEVEMENT_PROGRESS_COLLECTION_PAGE_RESOURCE = endpoints.ResourceContainer(
    AchievementProgressCollectionRequest
)
